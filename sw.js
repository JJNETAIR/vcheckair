 ...
 * Handles caching + background voucher expiry notifications
 */
...
self.addEventListener('fetch', (e) => {
  if (e.request.url.includes('google.com') || e.request.url.includes('format=csv') || e.request.url.includes('jsonbin.io')) return;
...
function getIndexedDBValue(key) {
  return new Promise((resolve) => {
    ...
    request.onupgradeneeded = (e) => { const db = e.target.result; if (!db.objectStoreNames.contains('alerts')) db.createObjectStore('alerts'); };
    request.onsuccess = (e) => {
...
self.addEventListener('periodicsync', (event) => {
  if (event.tag === 'check-voucher-expiry') event.waitUntil(scanAndTriggerVoucherNotifications());
});
            ...
            if (key.startsWith('alert_date_') && cursor.value === todayStr) {
              ...
              if (status === 'pending') {
                ...
                await self.registration.showNotification('Apple Air Wi-Fi 🔔', {
                  body: `Your internet voucher (${code}) expires tomorrow! Top up soon to avoid disconnection.`,
